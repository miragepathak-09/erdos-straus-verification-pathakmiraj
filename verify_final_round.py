...
This script certifies the material added in the final revision:
  ...
  I. THIN-SET CENSUS (coverage disclosure, Remark rem:density).
     Among n = 240k + 121 (resp. 169) with k < 5000, count the THIN
     values (all prime factors = 1 mod 4) split into squares vs.
     non-squares (and how many of the latter are prime).  Expect:
     27 vs. 2560 (1443 prime) for rho = 121, and 27 vs. 2541
     (1425 prime) for rho = 169.  Also the Type II witness at the
     thin non-square n = 65 (Open Problem op:lifting):
     4/65 = 1/17 + 1/390 + 1/6630 with 65 | 390, 65 | 6630,
     gcd(17, 65) = 1, and x = 17 = (65 + 3)/4.
...
expect = {121: (27, 2560, 1443), 169: (27, 2541, 1425)}
for rho, (e_sq, e_ns, e_nsp) in expect.items():
    n_sq = n_ns = n_ns_prime = 0
    for k_ in range(5000):
        n_ = 240 * k_ + rho
        if all(q_ % 4 == 1 for q_ in sp.factorint(n_)):
            rr, exact = sp.integer_nthroot(n_, 2)
            if exact:
                n_sq += 1
            else:
                n_ns += 1
                if sp.isprime(n_):
                    n_ns_prime += 1
    check((n_sq, n_ns, n_ns_prime) == (e_sq, e_ns, e_nsp),
          f"census mismatch for rho={rho}: "
          f"{(n_sq, n_ns, n_ns_prime)} != {(e_sq, e_ns, e_nsp)}")
    print(f"  rho = {rho}: {n_sq} thin squares vs. {n_ns} thin "
          f"non-squares ({n_ns_prime} of them prime)  OK")

# Type II witness at the thin non-square n = 65 (op:lifting)
check(es(17, 390, 6630, 65), "4/65 = 1/17 + 1/390 + 1/6630")
check(390 % 65 == 0 and 6630 % 65 == 0 and gcd(17, 65) == 1,
      "genuine Type II at the thin non-square 65 = 5*13")
check((65 + 3) // 4 == 17 and (65 + 3) % 4 == 0, "x = (n+3)/4 ansatz")
check(all(q_ % 4 == 1 for q_ in sp.factorint(65)), "65 is thin")
print("  4/65 = 1/17 + 1/390 + 1/6630: Type II at thin non-square,"
      " x = (65+3)/4 = 17  OK")
